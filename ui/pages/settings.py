...
class SettingsPage(QWidget):
    ...
    def __init__(self, context: AppContext, parent: QWidget | None = None) -> None:
        ...
        self.install_dll_btn = QPushButton("Installa / Aggiorna Mod Studio dxgi.dll nel Gioco")
        ...
        self.uninstall_dll_btn = QPushButton("Rimuovi Mod Studio dal Gioco")
        ...
        self.radio_sider_content = QRadioButton(
            "Workspace Mod Studio: <WorkspaceRoot>/content (Isolato da aggiornamenti Steam)"
        )
    ...
    def _refresh_metrics(self) -> None:
        ...
        if exe_valid and dll_installed:
            self.banner.set_message("Ambiente di gioco e Mod Studio configurati correttamente.", "success")
        elif exe_valid:
            self.banner.set_message(
                "eFootball rilevato. Installa o aggiorna la DLL dxgi.dll per attivare Mod Studio.", "info"
            )
        else:
            self.banner.set_message(
    ...
    def _install_sider_dll(self) -> None:
        ...
        try:
            ...
            self.banner.set_message(
                f"✅ Mod Studio dxgi.dll e sider.ini installati con successo in {dst_dir}!", "success"
            )
            self._refresh_metrics()
    ...
    def _uninstall_sider_dll(self) -> None:
        ...
        if dst.is_file():
            try:
                dst.unlink()
                self.banner.set_message("Mod Studio dxgi.dll rimosso dal gioco.", "info")
                self._refresh_metrics()
            except Exception as exc:
                self.banner.set_message(f"Impossibile rimuovere dxgi.dll: {exc}", "error")
        else:
            self.banner.set_message("Nessuna DLL Mod Studio presente nel gioco.", "info")
