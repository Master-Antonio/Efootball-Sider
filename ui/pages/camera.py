...
class CameraPage(QWidget):
    ...
    def __init__(self, context: AppContext, parent: QWidget | None = None) -> None:
        ...
        mode_row.addWidget(QLabel("Hook mode"))
        self.mode = QComboBox()
        self.mode.addItem("Telemetry (read-only, safe)", "telemetry")
        self.mode.addItem("Apply (write camera values)", "apply")
        self.mode.currentIndexChanged.connect(self._mark_dirty)
        mode_row.addWidget(self.mode, 1)
        controls.layout.addLayout(mode_row)

        self.legacy_hooks = QCheckBox("Install legacy unverified hooks (debug only)")
        self.legacy_hooks.toggled.connect(self._mark_dirty)
        controls.layout.addWidget(self.legacy_hooks)
    ...
    def save(self) -> None:
        settings = CameraSettings(
            ...
            legacy_hooks=self.legacy_hooks.isChecked(),
        )
    ...
    def _set(self, settings: CameraSettings) -> None:
        ...
        self.legacy_hooks.setChecked(settings.legacy_hooks)
