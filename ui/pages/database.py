...
class TeamEditDialog(QDialog):
    def __init__(self, record: dict, parent: QWidget | None = None) -> None:
        ...
        # Form Card Frame
        card = PageSection(
            "DATI SQUADRA & LICENZA", "Nome ufficiale e codice tri-lettera per tabelloni e telecronaca"
        )
        form = QFormLayout()
        form.setSpacing(12)
        form.setLabelAlignment(Qt.AlignmentFlag.AlignLeft)
...
class DatabasePage(QWidget):
    ...
    def __init__(self, context: AppContext, parent: QWidget | None = None) -> None:
        ...
        self.source_meta = QLabel(
            "dt200 definisce il database principale per le squadre offline ed esibizione"
        )
        ...
        if curr_tab == "players":
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                try:
                    ...
                    self.banner.set_message(
                        f"Giocatore #{record['player_id']} ({new_name}) salvato con successo!", "success"
                    )
            ...
            if dialog.exec() == QDialog.DialogCode.Accepted:
                ...
                try:
                    ...
                    self.banner.set_message(
                        f"Squadra #{record['team_id']} ({new_name} - {new_short}) salvata con successo!",
                        "success",
                    )
