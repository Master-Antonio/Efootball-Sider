...
class CameraSettings:
    ...
    # "telemetry" logs live camera values without writing; "apply" writes them.
    mode: str = "telemetry"
    legacy_hooks: bool = False
...
class ConfigurationService:
    ...
    def read_camera(self) -> CameraSettings:
        ...
        mode = values.get("mode", "telemetry").strip().lower()
        if mode != "apply":
            mode = "telemetry"
        legacy_hooks = values.get("legacy_hooks", "0").lower() in {"1", "true", "yes", "on"}
    ...
    def save_camera(self, settings: CameraSettings) -> None:
        values = {
            ...
            "mode": settings.mode,
            "legacy_hooks": "1" if settings.legacy_hooks else "0",
        }
