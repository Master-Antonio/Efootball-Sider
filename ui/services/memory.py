...
ASSET_PATTERN = re.compile(
    ...
    # stadium/kit ids must start on a token boundary, otherwise binary noise
    # like b"u1234" floods the results
    rb"(?<![A-Za-z0-9])(?:st[0-9]{3}|u[0-9]{4})[A-Za-z0-9_/\-.]*)",
    re.IGNORECASE,
)
...
class MemoryDiscoveryService:
    def __init__(self) -> None:
        # RLock: scan() holds the lock and calls attached(), which re-acquires it.
        # A plain Lock here deadlocked the discovery worker (and Stop with it).
            ...
            if readable:
                ...
                if read_ok:
                    ...
                    for match in ASSET_PATTERN.finditer(raw):
                        ...
                        # Drop ultra-short noise hits (binary junk like "u12").
                        if len(path) >= 5:
                            results.append(
                                AssetHit(
