...
fn find_sider_ini() -> Option<PathBuf> {
    ...
    for ini_name in &["modstudio.ini", "sider.ini"] {
        if let Some(dir) = get_module_dir() {
            candidates.push(dir.join(ini_name));
            if let Some(parent) = dir.parent() {
                candidates.push(parent.join(ini_name));
            }
        }
        candidates.push(PathBuf::from(*ini_name));
        candidates.push(PathBuf::from(format!("../{}", ini_name)));
        candidates.push(PathBuf::from(format!("../../{}", ini_name)));
    }

    if let Some(dir) = get_module_dir() {
        log_msg(&format!("[INI] Mod Studio DLL directory: {:?}", dir));
    }

    for c in &candidates {
        if c.exists() {
            log_msg(&format!("[INI] Found configuration at: {:?}", c));
            return Some(c.clone());
        }
...
fn worker_loop() {
    log_msg(&format!("=== eFootball Mod Studio Core {} Initializing Outside Loader Lock ===", SIDER_VERSION));
    ...
    if let Some(ref ini_p) = sider_ini_path {
        livecpk::load_livecpk_settings_from_ini(ini_p);
    }
    ...
    overlay::set_ini_path(sider_ini_path.clone());
    overlay::start_input_listener();
    // The OSD window is created on a delayed thread (see spawn_osd_window):
    // nothing heavy runs while the game boots.
    overlay::spawn_osd_window();
...
pub unsafe extern "system" fn DllMain(hinst: HMODULE, reason: u32, _reserved: *mut c_void) -> BOOL {
    if reason == 1 {
        ...
        logger::set_module_handle(hinst as isize);
        if is_game_process() {
