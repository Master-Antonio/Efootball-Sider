...
// The basename fallback can hijack any file that merely shares a name with a
// modded asset; it produced only self-redirect noise so far. Off by default,
// enable with `[LIVE_CPK] basename_fallback = 1` in sider.ini.
static BASENAME_FALLBACK_ENABLED: std::sync::atomic::AtomicBool =
    std::sync::atomic::AtomicBool::new(false);
const MAX_LOGGED_OVERRIDES: usize = 20;

pub fn load_livecpk_settings_from_ini(ini_path: &std::path::Path) {
    let Ok(file) = std::fs::File::open(ini_path) else {
        return;
    };
    ...
    for line in reader.lines().flatten() {
        ...
        if line.starts_with('[') && line.ends_with(']') {
            ...
            in_section = sec == "live_cpk";
            continue;
        }
        if in_section {
            if let Some((k, v)) = line.split_once('=') {
                if k.trim().eq_ignore_ascii_case("basename_fallback") {
                    let v = v.trim();
                    BASENAME_FALLBACK_ENABLED
                        .store(v == "1" || v.eq_ignore_ascii_case("true"), Ordering::Relaxed);
                }
            }
        }
    }
...
fn resolve_custom_path(requested_path: &str) -> Option<PathBuf> {
    ...
    if let Ok(guard) = VFS_MAP.read() {
        if let Some(ref vfs) = *guard {
            ...
            if BASENAME_FALLBACK_ENABLED.load(Ordering::Relaxed) {
...
mod tests {
    ...
    // VFS_MAP and BASENAME_FALLBACK_ENABLED are global; serialize the tests that mutate them.
    ...
    fn test_lookup_live_asset_basename_fallback() {
        let _guard = TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        BASENAME_FALLBACK_ENABLED.store(true, Ordering::Relaxed);
        let mut rel_map = HashMap::new();
        let mut basename_map = HashMap::new();
        rel_map.insert(
        ...
        BASENAME_FALLBACK_ENABLED.store(false, Ordering::Relaxed);
    }

    #[test]
    fn test_basename_fallback_disabled_by_default() {
        let _guard = TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        BASENAME_FALLBACK_ENABLED.store(false, Ordering::Relaxed);
        ...
        // Only a basename match is possible -> must NOT resolve while disabled.
        let res = resolve_custom_path("A:\\Games\\eFootball\\Paks\\SubDir\\Team.bin");
        assert!(res.is_none());
    }
