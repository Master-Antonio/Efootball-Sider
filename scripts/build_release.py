...
DIST = ROOT / "dist"
ARCHIVE = DIST / "eFootball_Mod_Studio.zip"
...
def stage_release() -> Path:
    ...
    for filename in (
        "Avvia_Mod_Studio.bat",
        "CHANGELOG.md",
        "CODE_OF_CONDUCT.md",
        "CONTRIBUTING.md",
        "Disinstalla_Mod_Studio.bat",
        "Installa_Mod_Studio_in_eFootball.bat",
        "LICENSE",
        "README.md",
        "Riavvia_eFootball_con_Mod_Studio.bat",
        "SECURITY.md",
        "pyproject.toml",
        "requirements.txt",
        "sider.ini",
        ...
        if src_file.exists():
            shutil.copy2(src_file, WORK / filename)
    shutil.copy2(source_dll, WORK / "dxgi.dll")
    copy_tree(ROOT / "ui", WORK / "ui")
    if (ROOT / "bin").is_dir():
        copy_tree(ROOT / "bin", WORK / "bin")
