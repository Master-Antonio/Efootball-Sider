...
class TestQtApplication(unittest.TestCase):
    ...
    def test_main_window_contains_all_operational_pages(self):
        window = MainWindow()
        expected_types = (
            OverviewPage,
            AssetsPage,
    ...
    def test_mods_page_sync_buttons_and_columns(self):
        window = MainWindow()
        mods_page = None
        for i in range(window._stack.count()):
            widget = window._stack.widget(i)
            if isinstance(widget, ModsPage):
                mods_page = widget
                break
        self.assertIsNotNone(mods_page)
        self.assertTrue(hasattr(mods_page, "sync_all_button"))
        self.assertTrue(hasattr(mods_page, "pak_sync_button"))
        self.assertEqual(mods_page.sync_all_button.text(), "Sync All to ~mods")
        self.assertEqual(mods_page.pak_sync_button.text(), "Auto-Build All Triplets")

        # Verify sync_state column in both tables
        col_names = [col.key for col in mods_page.model.columns]
        self.assertIn("sync_state", col_names)
        pak_col_names = [col.key for col in mods_page.pak_model.columns]
        self.assertIn("sync_state", pak_col_names)

        # Trigger sync_all_to_mods when no compatible packages exist - should show banner
        mods_page.sync_all_to_mods()
        self.assertIn("Nessun pacchetto", mods_page.banner.label.text())
        window.close()
