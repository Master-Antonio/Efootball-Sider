...
### 4.1 In-Memory Team & Squad Synchronization (`teams.rs`)
In addition to offline CPK repacking, Sider can patch loaded Fox Engine database records directly in process memory at runtime:
- **String Table Scanner**: Iterates committed heap memory with `VirtualQuery` and `ReadProcessMemory` searching for loaded team names (ASCII and UTF-16 representations).
- **In-Place Mutation**: Rewrites team names and short names with `WriteProcessMemory`, ensuring exact null-byte termination and length boundary enforcement to prevent memory corruption.
